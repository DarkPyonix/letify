...
Everything letify writes on a remote machine lives under one directory per account, the workspace root: the project `.venv`, materialized volume data, temporary files and the worker's working directory. An account in `~/.letify/config.toml` sets it with `workspace = "<path>"`, a path that starts with `/` or `~`, expanded on the machine. A project file cannot set it. The default is `~/.letify-runtime` for `shell`, `tunnel` and `elice`, `/content/letify` for `colab`, and `/letify` for `modal`, where a Modal volume is mounted so the root persists across sandboxes. `letify login shell` and `letify login tunnel` ask for it, take `--workspace PATH` in a script, and check over SSH that the path can be created and written without root before anything is written. `letify check <alias>` reports whether the root is writable.
...
key = "~/.ssh/id_ed25519"
persistent = true
workspace = "/workspace/researcher/letify"
...
`letify usage` reads each account from its own service: Colab compute units from `ccu-info`, Elice credit in KRW from the billing API at `billing_endpoint`, Modal's month against `monthly_credit` (30 USD by default), and Kaggle GPU hours per week. A machine reached by SSH has no quota. Providers are asked at once, each waited on for `usage_timeout` seconds (20 by default). An entry can instead name a command that prints the figure: `usage_command` is run when `letify usage` asks, `usage_unit` names what it counts and `usage_limit` gives the ceiling. The last number in the output is read as the remaining amount. Without a command, `usage_limit` is the plan allowance for the provider's own reading, which gives a Colab balance a percentage. The output is one block per account: a gauge of used against the limit, the amount left, a countdown to the reset and the hourly rate, coloured only on a terminal without `NO_COLOR`.

...
Specified, not implemented yet: the call is meant to start as soon as its first wave of data is on the runtime, with the rest arriving in the background while the call runs. The first wave comes from what letify can see in the pickled call, a `Dataset` or a path list in the arguments and the reads in the function's code. After the call starts the order comes from the runtime watching the real reads, mainly by reading a `DataLoader`'s sampler many batches ahead. Today every missing file is uploaded before the call is sent, so a 100 GiB dataset waits about 20 minutes before the first step, at 85 to 90 MiB/s.

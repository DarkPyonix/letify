...
Specified, not implemented yet: the call is meant to start as soon as its first wave of data is on the runtime, with the rest arriving in the background in an order letify derives from the call itself, from a `Dataset` or path list in the arguments and from the reads in the function's code. Today every missing file is uploaded before the call is sent, so a 100 GiB dataset waits about 20 minutes before the first step, at 85 to 90 MiB/s.

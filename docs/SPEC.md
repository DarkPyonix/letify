...
The order is decided before the call is sent, by analysing what the local process already holds. Nothing is decided by waiting for the body to ask.
...
With the call, the local process sends one `data_manifest` request holding, per detected path, every file's relative POSIX path, digest and size, and whether the runtime already holds that digest. The worker keeps it for the life of the call. It is what makes a directory listing complete before any byte of it has arrived, which What the body sees before a file arrives describes.
...
- **Streaming project data is specified but not implemented.** The send order, the first wave, the background sender, the manifest backed listing and the blocking backstop of Project data are a decision recorded here, not yet code. Today the local process uploads every missing blob before it sends the call, so a first step waits for the whole dataset: about 12 s for 1 GiB and about 20 minutes for 100 GiB, at the 85 to 90 MiB/s measured on the project's links.

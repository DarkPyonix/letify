...
> A Kaggle account is declared with the browser session cookie of a logged-in kaggle.com tab. It opens no tunnel, port forward or Tailcat link, so the kernel channel is its only link to a session, and that channel cannot carry a device stream, so it cannot serve `host="local"`.

Not opening a link is a decision of this project, not a rule of Kaggle's. Two sources bear on it, and they say different things. Kaggle staff answered a request for SSH access with "You can't run docker inside Kaggle. We also don't support port forwarding.", which is a statement that the feature does not exist, not that reaching it is forbidden. The Acceptable Use Policy of June 22, 2025 forbids using the Services "to disable, interfere with or circumvent any aspect of the Services" and lists "tools for circumvention/obfuscation" among abusive uses. Whether hole punching to a machine that offers no inbound port is circumvention is not written anywhere, so treating it as circumvention is this project's reading rather than Kaggle's rule. letify takes the cautious side because the cost of being wrong falls on the user's account. The session has outbound network, so a link dialled out from the session to the local process is possible in principle; letify does not open one, and `host="local"` is refused for the throughput reason in Placements a provider cannot serve, not because Kaggle forbids it.
...
1. **Start.** `open_channel` reads the account's cookie, refusing the run with a `ConfigError` when it is missing or past its `exp`. The run is started with the session's internet access enabled (`compute.internet.isEnabled` in `CommitAndRun`), because the environment step downloads from PyPI and the uv installer from astral.sh; Kaggle grants that access only to a phone-verified account, and a run it refuses fails with Kaggle's own error. It runs the token chain to get the routed URL, waits for `<base>/api/status` to answer, then creates one kernel with `POST <base>/api/kernels` and body `{"name": "python3"}` and keeps its id for the runtime. `stop` deletes the kernel with `DELETE <base>/api/kernels/<id>`, then cancels the run, both best effort.
2. **The worker.** The session carries one worker for the life of the runtime, not one program per request. The Kaggle adapter, `letify/providers/kaggle_adapter.py`, is a bridge process started by `uv run --no-project --python 3.13 --with "jupyter-kernel-client<1" python -P`, and it receives the URL and the kernel id in the environment variables `LETIFY_JUPYTER_URL` and `LETIFY_KERNEL_ID`. It executes one cell in that kernel which runs until the runtime ends, and that cell carries a shim rather than the bootstrap stub of Channels. A kernel answers `input()` and nothing else: the cell's `sys.stdin` is a `TextIOWrapper` whose `buffer` reads empty instead of raising `input_request`, so a worker reading binary from standard input would read nothing at all. The shim therefore reads base64 lines with `input()`, decodes each one, and writes the bytes into a pipe the worker reads as its standard input, with the worker's own standard output going back out as `stream` messages. The bridge moves the lines on two threads, one per kernel channel, and never on one: what it reads on its standard input one thread hands to the cell by answering `input_request`, and every `stream` message the cell writes the other thread copies to its standard output. One thread is required rather than a convenience. Answering an `input_request` from the same loop that reads the cell's output blocks that loop in a standard-input read, and while it blocks no `stream` message is delivered. The worker announces itself with a `stream` message, so a single-loop bridge holds that hello behind an `input_request` the channel does not answer until it has seen the hello, which is a deadlock, and the same deadlock returns on every large response, where the cell reads the next request while the worker is still writing output. The letify process never imports `jupyter_kernel_client`.
   ...
   The channel is a `FramedChannel` with `text_frames` set, so the worker writes each frame as a base64 line, as the Modal sandbox already does for the same reason: a kernel's `stream` messages carry text, not bytes. `persistent_channel` is therefore true, which is what gives Kaggle an object table, a blob table that keeps a large argument across calls, and the option of `host="local"` if a link ever makes it worth using. The bridge's standard input is an unbuffered pipe, and a blocking write into a full pipe returns short when a signal arrives while it waits, as `SIGCHLD` does when a process with a handler for it, such as one where PyTorch has forked data loader workers, sees an earlier worker exit. So the channel writes each base64 line until every byte of it is in the pipe, and counts the piece as sent only then.
...
3. **Failure.** The bridge exiting is the worker dying, so every blocked read ends as a lost channel does, with the bridge's standard error as the detail. The provider then reads `<base>/api/status` once. When that read answers 200 the session is alive and the worker is not, so `RuntimeFailure` is raised with the bridge's standard error. Otherwise `KaggleSessionEnded` is raised. It is a `RuntimeLost`, and its message does not state that the session ended, because letify cannot tell an ended session from one that failed to start: the proxy answers 404 for both, and for a session id that never existed. The message says the session did not answer, names the causes it could be (20 minutes idle, the 12 hour limit, or a failure to start) and tells the user to run the function again. A session Kaggle ends mid call is therefore reported as the lost runtime it is, rather than as the call's own failure. A worker that letify's own startup watchdog killed, because it did not say hello within the startup timeout, is reported as that, naming the timeout, with the bridge's standard error attached: the death is letify's doing, not the session's, and the report must not read as a crash inside an answering session.
...
4. A machine with status `started` is used as it is. A machine with status `idle` is started with `eci compute vm start <name>`. Any other status is a transition, and `eci compute vm get <name> --format json` is read every 10 seconds until it is `idle`, for up to 300 seconds, before the start. After a start the status is read every 10 seconds until it is `started`, for up to 600 seconds. Either limit raises `RuntimeFailure` naming the last status, except when that status is `queued`: Elice has not placed the machine on a host, which is a capacity answer like a refused spot launch, so it raises `ProviderUnavailable` naming the machine and its instance type, and the machine is ended as A start that fails describes.
...
`Kaggle` sets it, because the kernel channel is the only link letify opens to a session: Kaggle offers no inbound port and letify dials no link out of the session (see Kaggle). That channel carries base64 lines through a proxy at about 9 MiB/s up and 1.5 MiB/s down, measured from Seoul, which cannot carry a device stream. It is a throughput limit of the link letify has, not a rule of Kaggle's. The refusal happens in two places:
...
A stream may also carry more than one request. The local process sends them in order on one stream without waiting for a reply between two of them, and the worker answers each with one reply, in order, on that stream. The local process keeps at most 8 of them unanswered, so the worker holds at most that many it has not yet served, and it reads the replies whenever it has to wait for room and once more before it releases the stream id. A failed reply ends the sequence and raises. `data_put`, which [Where the bytes come from](#project-data-transfer) describes, is the only request sent this way.
...
Over the channel, a file travels in pieces of at most 8 MiB, each one a `data_put` request carrying the piece as an out-of-band `DATA` frame, its offset, and whether it is the file's last. Every piece of a call, across all of its files, is sent on one stream in send order, and the local process does not wait for a reply between two pieces: it keeps at most 8 pieces unanswered, which is 64 MiB in flight, as Waiting for a reply describes. So the link carries the next piece while the worker writes the one before, and a small file costs no round trip of its own. The worker appends each piece to `<digest>.partial.<pid>` in arrival order and answers it, and after the last piece of a file checks the digest of the whole file and renames it into the cache. A digest that does not match fails that piece's reply, which raises `RuntimeFailure` on the local side, and nothing is renamed. The local process reads every outstanding reply before it sends the call, so the first wave is placed when the body starts.
...
The background sender then sends the blobs that are not yet placed, in send order, as the pipelined `data_put` pieces Where the bytes come from describes, on the same stream the first wave used. It runs on the session's channel beside the call, and a blob is never sent twice. A digest the worker asks for with `data_want` goes out after the pieces already in flight, at most 64 MiB, and before any file the sender has not started, so a blocking read is answered ahead of the rest of the queue but never interrupts a file in the middle.

A call whose files the runtime already holds streams nothing, and pays for none of this. The local process sends such a call with no manifest, no send order and no observation settings, and asks for no wait statistics when it ends. The worker then places every file before the body starts, writes no manifest file for a spawned child and wraps no data loader, because each of them could only answer that every file is already there. The patch stays installed, because Copy on write answers from it for the links the call placed, and it costs one path check per open. This is the repeat run of a dataset a researcher is iterating on, so it is the common case rather than an edge one, and the streaming machinery must not tax it. The data log line still reports what was detected and what was already on the runtime.
...
| `builtins.open`, `io.open`, `os.open` | sends `data_want` and blocks that caller until the blob is placed, then opens the real file | opens the real file, after the conversion of Copy on write when the mode writes |
...
Each call that carries data gets a call directory, `<workspace root>/data/calls/<call id>`, where the call id is 16 random hex characters. The `n`th distinct detected path, counted from 0, is placed at `<call directory>/<n>/<name>`, where `<name>` is the local path's final component. A file is placed there; a directory is recreated there with every manifest entry at its relative path. For a path that does not exist, only `<call directory>/<n>` is created, so the body may create a file or a directory at the runtime path. Every entry, an input-only file and a file inside a directory alike, is a hard link to the cache file, or a copy where a hard link fails. A link costs one directory entry where a copy costs the file's bytes: placing a warm 10 GiB directory of 2560 files as copies cost 14 s of every call on dept_gpu and 6 s on the Local provider, and placing it as links costs under 0.1 s, measured in the copy on write experiment pull request. The body may still rewrite a file inside a directory, because the link becomes a private copy the moment the body opens it for writing, which Copy on write describes. The same local path detected twice in one call maps to the same runtime path.
...
#### Copy on write <!-- id: project-data-copy-on-write -->

A placed link becomes a private copy when the body opens it for writing, so no write of the body reaches the cache file and the copy is what Writing back sees.

The worker installs the patch of What the body sees before a file arrives for every call that placed at least one link, not only for a call with files still arriving. When the body calls `builtins.open` or `io.open` with a mode containing `w`, `a`, `x` or `+`, or `os.open` with `O_WRONLY`, `O_RDWR`, `O_APPEND` or `O_TRUNC`, on a path the call placed as a link, the worker copies the cache file to `<name>.letify-placing.<pid>` beside it, renames the copy over the link with `os.replace`, and only then opens the file. `pathlib.Path.open`, `write_bytes` and `write_text` call `io.open`, so they are covered. The copy has the process's default file mode rather than the cache file's read-only mode, so a body that is not root can write it. A file the body only reads stays a link. A path is converted once, so a second open for writing finds the copy, and when two threads open the same link for writing at once, one converts while the other waits for it. The cache file's link count drops by one at the conversion, so Runtime data cache budget treats it as any other unlinked blob from then on.

A converted file is hashed after the call whatever its inode, size and modification time say, because the file system's timestamp granularity can be coarser than the time between the copy and the body's first write, and a write of the same size inside that window would otherwise pass for unchanged.

The patch covers the process that runs the body and everything it forks. A process the body starts fresh, a spawned interpreter or a program such as `sh` or `ffmpeg`, sees the link itself. Its write is refused by the cache file's read-only mode when the process is not root. When the process can write despite the mode, the bytes land in the cache file: the write-back copies the changed file out of the link first, so the local path receives the change, and the check after the call of Writing back then removes that cache file, so the next call sends the digest again. This is the accepted gap. The alternative, a writable copy of every file on every call, is what the 20 s paid for.
...
On the runtime, after the body returned and before the call directory is removed, the worker walks each output's runtime path as Which paths are data walks a directory, without following symbolic links. A file is unchanged when it is the hard link or copy the worker placed and its inode, size and modification time in nanoseconds equal those recorded when it was placed; it is not read. Any other regular file, a file Copy on write converted included, is hashed with the file digest, and a file whose digest equals the digest placed at that relative path is unchanged too. Each changed file is renamed into the file blob cache and made read-only, or dropped when the cache already holds its digest with its size. The worker keeps the list of relative path, digest and size per output under the call id until the local process asks for it with one `data_written` request, which removes it.
...
A file placed as a hard link shares its inode with the cache file. When a process writes into such a file in place, which Copy on write leaves possible for a process the body starts fresh and running as root, the cache file no longer matches its digest. After every call, whether it returned or raised, the worker compares each placed cache file's size and modification time with those recorded at placement and removes the cache file when they differ, so the next call receives the digest again.
...
A dial the kernel refuses at once is not a connection. This happens when the peer's SYN has already reached the listener and taken the port pair, so `connect` fails with `EADDRNOTAVAIL` before any packet is sent, and the socket then reports itself writable with no pending error. Such a socket is closed and the dial is repeated 0.2 s later, as a refused connection is, and the punch waits for the listener to hand over the connection the peer is completing.

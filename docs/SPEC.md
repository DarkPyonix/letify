...
3. **Failure.** The bridge exiting is the worker dying, so every blocked read ends as a lost channel does, with the bridge's standard error as the detail. The provider then reads `<base>/api/status` once. When that read answers 200 the session is alive and the worker is not, so `RuntimeFailure` is raised with the bridge's standard error. Otherwise `KaggleSessionEnded` is raised. It is a `RuntimeLost`, and its message does not state that the session ended, because letify cannot tell an ended session from a registered URL that no longer routes: the proxy answers 404 for both, and it answers 404 for a session id that never existed. The message says the session did not answer, names the causes it could be (20 minutes idle, the 12 hour limit, or a registered URL that no longer routes) and tells the user to start a new session with Run, Kaggle Jupyter Server and run `letify login kaggle <alias> --connect <new URL>`. A session Kaggle ends mid call is therefore reported as the lost runtime it is, rather than as the call's own failure.
...
6. **The adapter's two modes.** `LETIFY_ADAPTER_MODE` names one: `bridge`, the default, is everything above, and `program` runs one program in the kernel and exits. Recording the session's devices at login is the only caller of `program`, because it happens before any runtime exists and so has no channel to carry frames. The mode is its own variable and is never inferred from whether another one is set: a duration that also decides which mode runs carries two meanings in one name, which is what let a provider skip a check it should have run.

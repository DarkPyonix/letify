...
def test_an_account_with_no_registered_session_runs_in_batch_mode(isolated_home) -> None:
    from letify.providers.kaggle import BatchChannel

    provider = kaggle_provider()
    assert provider.prepares_env is False
    channel = provider.open_channel(type("R", (), {"name": "n", "instance": provider.CPU})())
    assert isinstance(channel, BatchChannel)
...
# -- Spec: Kaggle batch mode ---------------------------------------------------------


class FakeKaggleBatch:
    """The Kaggle CLI's kernels commands, answered without an account.

    ``push`` keeps the pushed ``script.py`` and metadata. ``status`` answers the listed states
    in order. ``output`` runs the pushed script in a local interpreter and writes its output as
    the JSON kernel log Kaggle produces, so the driver programs letify pushes are the real ones.
    """

    def __init__(self, states: list[str], config_view: str = "- username: researcher\n"):
        import subprocess
    ...
    def __call__(self, command, **kwargs):
        ...
        # uv tool run ... --from kaggle kaggle <verb>: the verb follows the executable.
        tail = command[command.index("--from") + 3 :] if "--from" in command else command
        verb = " ".join(tail[:2])
        if verb in self.fail:
            return FakeCompleted(returncode=1, stderr=f"{verb} refused for {ACCESS_TOKEN}")
        if tail[:2] == ["config", "view"]:
            return FakeCompleted(stdout=self.config_view)
        if tail[:2] == ["kernels", "push"]:
            folder = Path(tail[tail.index("-p") + 1])
            self.pushes.append(
                {
                    "args": tail,
            ...
            return FakeCompleted(stdout="Kernel version 1 successfully pushed.\n")
        if tail[:2] == ["kernels", "status"]:
            state = self.states.pop(0) if len(self.states) > 1 else self.states[0]
            text = f'{tail[2]} has status "{state}"\n'
            if self.failure:
                text += f'Failure message: "{self.failure}"\n'
            return FakeCompleted(stdout=text)
        if tail[:2] == ["kernels", "output"]:
            folder = Path(tail[tail.index("-p") + 1])
            slug = tail[2].split("/", 1)[1]
            ran = self.real_run(
                [sys.executable, "-c", self.pushes[-1]["script"]],
                capture_output=True,
                text=True,
                timeout=120,
            )
            log = [{"stream_name": "stdout", "time": 1.0, "data": ran.stdout}]
            if ran.stderr:
                log.append({"stream_name": "stderr", "time": 1.1, "data": ran.stderr})
            (folder / f"{slug}.log").write_text(json.dumps(log), encoding="utf-8")
            return FakeCompleted()
        return FakeCompleted(returncode=2, stderr=f"unexpected {tail}")


@pytest.fixture
def kaggle_batch(isolated_home, patch_which, monkeypatch):
    from letify import tools
    from letify.providers import kaggle as kaggle_module

    patch_which(tools, present=True)
    monkeypatch.setattr(kaggle_module, "sleep", lambda seconds: None)

    def build(states: list[str], **kwargs):
        fake = FakeKaggleBatch(states, **kwargs)
        monkeypatch.setattr(kaggle_module.subprocess, "run", fake)
        return fake

    return build


def batch_channel(accelerator: str = "CPU", **options):
    from conftest import provider_of

    from letify.providers import Kaggle

    provider = provider_of(Kaggle, "kaggle_a", **options)
    instance = provider.device(accelerator)
    runtime = type("R", (), {"name": "letify-T4_1", "instance": instance})()
    return provider, provider.open_channel(runtime)


def test_a_batch_call_is_one_push_carrying_the_programs_held_before_it(kaggle_batch) -> None:
    fake = kaggle_batch(["KernelWorkerStatus.RUNNING", "KernelWorkerStatus.COMPLETE"])
    _provider, channel = batch_channel()
    channel.request({"op": "exec", "source": "import os\nos.environ['LETIFY_PREPARED'] = '1'\n"})
    assert fake.pushes == []
    value, _logs = channel.request(
        {"op": "eval", "source": "import os\n__letify_value__ = os.environ['LETIFY_PREPARED']"}
    )
    assert value == "1"
    assert len(fake.pushes) == 1
    verbs = [" ".join(c[c.index("--from") + 3 :][:2]) for c in fake.commands]
    assert verbs.count("kernels status") == 2
    assert verbs.count("kernels output") == 1


def test_the_pushed_kernel_is_a_private_cpu_script_named_after_the_runtime(kaggle_batch) -> None:
    fake = kaggle_batch(["complete"])
    _provider, channel = batch_channel()
    channel.request({"op": "eval", "source": "__letify_value__ = 1"})
    meta = fake.pushes[0]["meta"]
    assert meta["id"] == "researcher/letify-letify-t4-1"
    assert meta["title"] == "letify-letify-t4-1"
    ...
    args = fake.pushes[0]["args"]
    assert args[args.index("--timeout") + 1] == "1800"
    assert "--accelerator" not in args


@pytest.mark.parametrize(
    ("accelerator", "shape"),
    [("T4", "NvidiaTeslaT4"), ("P100", "NvidiaTeslaP100"), ("TPU_V3_8", "Tpu1VmV38")],
)
def test_an_accelerator_is_asked_for_by_its_kaggle_machine_shape(
    kaggle_batch, accelerator: str, shape: str
) -> None:
    fake = kaggle_batch(["complete"])
    _provider, channel = batch_channel(accelerator)
    channel.request({"op": "eval", "source": "__letify_value__ = 1"})
    args = fake.pushes[0]["args"]
    assert args[args.index("--accelerator") + 1] == shape


def test_the_timeout_is_the_smaller_of_the_account_and_the_call(kaggle_batch) -> None:
    fake = kaggle_batch(["complete"])
    _provider, channel = batch_channel(batch_timeout=900)
    channel.request({"op": "eval", "source": "__letify_value__ = 1"}, timeout=300)
    args = fake.pushes[0]["args"]
    assert args[args.index("--timeout") + 1] == "300"


def test_the_username_comes_from_kaggle_json_before_asking_the_cli(kaggle_batch) -> None:
    from letify.config.secrets import write_secret

    write_secret("kaggle_a", "kaggle.json", json.dumps({"username": "legacy-user", "key": "k"}))
    fake = kaggle_batch(["complete"], config_view="- username: someone-else\n")
    _provider, channel = batch_channel()
    channel.request({"op": "eval", "source": "__letify_value__ = 1"})
    assert fake.pushes[0]["meta"]["id"].startswith("legacy-user/")
    assert not any("config" in c for c in fake.commands)


def test_a_kernel_that_errors_is_a_runtime_failure_with_its_message(kaggle_batch) -> None:
    import letify

    fake = kaggle_batch(["KernelWorkerStatus.ERROR"])
    fake.failure = "Your notebook tried to allocate more memory than is available"
    _provider, channel = batch_channel()
    with pytest.raises(letify.RuntimeFailure, match="allocate more memory"):
        channel.request({"op": "eval", "source": "__letify_value__ = 1"})
    assert len(fake.pushes) == 1


def test_a_kernel_that_never_finishes_is_given_up_without_pushing_again(
    kaggle_batch, monkeypatch
) -> None:
    import letify
    from letify.providers import kaggle as kaggle_module

    clock = iter(float(n * 400) for n in range(100))
    monkeypatch.setattr(kaggle_module, "monotonic", lambda: next(clock))
    fake = kaggle_batch(["queued"])
    _provider, channel = batch_channel(batch_timeout=600)
    with pytest.raises(letify.RuntimeFailure, match="did not finish"):
        channel.request({"op": "eval", "source": "__letify_value__ = 1"})
    assert len(fake.pushes) == 1


def test_a_refused_push_names_the_command_and_hides_the_token(kaggle_batch) -> None:
    import letify
    from letify.config.secrets import write_secret

    write_secret("kaggle_a", "access_token", ACCESS_TOKEN)
    fake = kaggle_batch(["complete"])
    fake.fail.add("kernels push")
    _provider, channel = batch_channel()
    with pytest.raises(letify.RuntimeFailure) as caught:
        channel.request({"op": "eval", "source": "__letify_value__ = 1"})
    assert "kernels push" in str(caught.value)
    assert ACCESS_TOKEN not in str(caught.value)


def test_batch_mode_refuses_file_transfer(kaggle_batch) -> None:
    import letify

...
def test_an_account_with_no_registered_session_names_the_login_command(isolated_home) -> None:
    import letify

    provider = kaggle_provider()
    with pytest.raises(letify.ConfigError, match="letify login kaggle kaggle_a --connect"):
        provider.open_channel(type("R", (), {"name": "n"})())

...
That login is the whole setup. With no session registered, each call runs as one pushed Kaggle script kernel with a hard timeout (`batch_timeout`, 1800 s by default): nothing is opened in a browser and no URL is pasted. The kernel is the Kaggle image, so it brings its own Python and packages, and letify has to run on the same Python minor version as that image, 3.12 as of 2026-09-16. Shipping files to the kernel is the one thing this path cannot do.

To get file transfer and your own environment, run on a live session instead: start one in the Kaggle editor with Run, Kaggle Jupyter Server, copy its Colab Compatible URL, and run `letify login kaggle kaggle_a --connect '<URL>'`. That URL exists only in the editor, because Kaggle publishes no API that starts an interactive session, so this is the one step that needs a person. letify records the session's GPUs and runs calls there until Kaggle ends the session, after 20 minutes idle or 12 hours. Then it raises an error telling you to register a new URL, and it never keeps a session alive.
